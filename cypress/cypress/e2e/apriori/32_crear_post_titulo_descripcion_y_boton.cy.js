describe('Funcionalidad: Crear post', () => {
    ...
    it('Crear post con titulo, descripcion, divider y boton', ()=>{
        ...
        cy.wait(5000)
        cy.get('.gh-editor-title.ember-text-area.gh-input.ember-view').type(dataPool.title_climate_change)
        cy.wait(3000)
        cy.get('.kg-prose').click()
        cy.wait(3000)
        cy.get('.kg-prose').type(dataPool.divider)
        cy.wait(3000)
        cy.get('.kg-prose').type(dataPool.short_text)
        cy.wait(3000)
        cy.get('.kg-prose').type(dataPool.button)
        cy.wait(3000)
        cy.get('input[placeholder="Add button text"]').type(dataPool.button_title)
        cy.wait(2000)
        cy.get('input[placeholder="https://yoursite.com/#/portal/signup/"]').type(dataPool.link)
