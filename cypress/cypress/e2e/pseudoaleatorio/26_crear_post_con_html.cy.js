describe('Funcionalidad: Crear post', () => {
    ...
    it('Crear post con titulo y html', ()=>{

        //When I create a new post with a title and html
        cy.wait(2000)
        ...
        cy.wait(5000)
        cy.get('.kg-prose').type('/html{enter}')
        cy.wait(5000)
        cy.get('div[class="cm-line"]').type(`<html>
        <head>
        <title>Page Title</title>
        </head>
        <body>
        <h1>${dataPool.long_description}</h1>
        <p>${dataPool.short_description}</p>
        </body>
