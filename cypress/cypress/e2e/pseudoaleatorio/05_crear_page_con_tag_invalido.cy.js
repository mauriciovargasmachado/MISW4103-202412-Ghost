describe('Funcionalidad: Crear páginas', () => {
  ...
  it('Crear una página con tag inválido.', () => {
    ...
    cy.wait(1000)
    cy.get('.ember-power-select-trigger-multiple-input').first().type(dataPool.tag_invalid)
    cy.wait(1000)
    ...
    // Then I expect to see <PUBLISH_PAGE_INVALID_MESSAGE>
    cy.get('p.error').then(($p) => {
      expect($p.text()).includes(Cypress.env('PUBLISH_PAGE_INVALID_MESSAGE'))
    })
  })
})