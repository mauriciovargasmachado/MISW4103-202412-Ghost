describe('Funcionalidad: Crear tag', () => {
    beforeEach(() => {
      ...
      cy.wait(2000)
      cy.screenshot('create_tag_scenario_3/1', {overwrite: true})

      //And I fill input form with GHOST_USERNAME and GHOST_PASSWORD
      cy.get('input.gh-input.email').type(Cypress.env('GHOST_USERNAME'))
      cy.screenshot('create_tag_scenario_3/2', {overwrite: true})
      cy.get('input.gh-input.password').type(Cypress.env('GHOST_PASSWORD'))
      cy.screenshot('create_tag_scenario_3/3', {overwrite: true})
      cy.get('button.login.gh-btn.gh-btn-login.gh-btn-block.gh-btn-icon.ember-view').click()
      cy.wait(2000)
      cy.screenshot('create_tag_scenario_3/4', {overwrite: true})
      ...
      cy.screenshot('create_tag_scenario_3/5', {overwrite: true})
    })
    it('Crear un tag con descripción valida <=500 caracteres.', ()=>{
        ...
        cy.screenshot('create_tag_scenario_3/6', {overwrite: true})
        cy.wait(2000)
        cy.get('#tag-name').type('Test1')
        cy.wait(3000)
        cy.screenshot('create_tag_scenario_3/7', {overwrite: true})
        cy.get('[placeholder="15171A"]').type('ff0080')
        cy.wait(2000)
        cy.screenshot('create_tag_scenario_3/8', {overwrite: true})
        ...
        cy.wait(2000)
        cy.screenshot('create_tag_scenario_3/9', {overwrite: true})
